...
example~\cite{dropbox}. It supports many sharing features which don't
integrate well with traditional single user keys. A more robust,
...
\label{sec:intro}
The idea of keeping things safe from prying eyes is nothing complicated.
However, to the layman, the intricacies of
actually protecting data online are somewhat out of reach. This problem
...
files while still being able to normally use Dropbox? 
...
retrieving secrets, in this case, keys~\cite{custostrios}. The SSaaS model supports the features of
